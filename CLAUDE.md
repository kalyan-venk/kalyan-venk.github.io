...
- **2026-07-24: hover magnifier REMOVED, and the 3 standalone blueprint pages
  DELETED — the maps now live inline on each project page.**
  - **The magnifier is gone.** Kalyan's call: the labels are readable at normal size, so
    the 2.4x hover lens was doing nothing. Removed the `magnifier()` function, its call in
    `mount()`, the `.lens*` rules, `.dgm-wrap.zoomready svg.dgm{cursor:none}` and the
    `cursor:crosshair` that paired with it, plus the `.hint-pill` that advertised it. The
    diagram-bar chip now reads "hover a chip to spotlight its stage" (crosshair icon, not a
    magnifying glass) — the **chip-spotlight feature is separate and still live**. Copy that
    promised zooming was purged: the hero pill, the `<meta description>`s ("2 live, zoomable
    blueprints" -> "2 live blueprints") and the project pages' "Hover anywhere to magnify."
  - **`agentic-llmops-blueprint.html`, `inference-lens-blueprint.html` and
    `predictops-blueprint.html` are deleted.** Their two diagrams (plus section copy, legends
    and read-along notes) were moved verbatim into the matching project page's `#blueprints`
    section, replacing the 2 hand-drawn preview thumbnails that used to link out. The
    anchors moved with them: `/agentic-llmops#system` and `#code` (was
    `/agentic-llmops-blueprint#system`). **Do not recreate the blueprint pages.**
  - **The engine is now a shared asset**, not 3 inlined copies: `assets/blueprint.js`
    (`KVBlueprint.mount(svgId, wrapId, spec, stepsKey)`) + `assets/blueprint.css`. Only the
    per-project `sys`/`code` **specs** stay inline on each page, since that is the only
    per-page part. Total shipped bytes for this content: **214KB -> 135KB (-37%)**, and the
    engine is cached after the first project page. This also removes the old reason these
    pages were self-contained (CLAUDE.md used to say "the magnifier is the piece `flow.js`
    does not have") — with the lens gone, there was nothing left to justify the duplication.
  - **Rendering is LAZY.** `mount()` reserves the diagram's height via
    `--dgm-ar` (an `aspect-ratio` on `.dgm-wrap:not(.rendered)`) and only builds the nodes,
    edges and packet routes when the diagram comes within 400px of the viewport. Each map is
    ~350 SVG elements plus a `getTotalLength()` per edge (which forces layout), and a project
    page now carries two, so building at load would cost first paint for below-the-fold
    content. The reserved aspect ratio is what stops the lazy build from shifting the page —
    **keep it if you touch `mount()`**. Motion still follows visibility (a second observer)
    and `visibilitychange`.
  - **⚠️ EVERY rule in `blueprint.css` is scoped under `.bpx`, deliberately.** `style.css`
    owns `.card`, `.pkt`, `.legend`-adjacent and `.tag` for other pages; an unscoped rule
    here silently restyles the project cards and the job-scanner flow legend. For the same
    reason the read-along cards are `.bp-note`, not `.card`, and the legend's square swatch
    is `.k.swatch`, not `.k.node` (`style.css` has an unscoped
    `.node{padding:16px 20px;min-width:120px}` that stretched it into a wide empty box — see
    `MISTAKES.md`).
  - **Fixed while moving:** all 3 blueprints had been generated by `cp
    inference-lens-blueprint.html`, so the agentic and predictops maps carried
    `aria-label="System design of Inference-Lens"` / `"Code map of Inference-Lens"`. Each now
    names its own project. Also dropped dead engine code (a `KIND` placeholder holding the
    garbage value `"#3a4karma"`), made `animate()` stop mutating `spec.flow` when it adds the
    muted fallback routes (so a re-mount cannot accumulate duplicates), and scoped
    `buildSteps()`'s `.flowsteps` lookup to the diagram's own `.dgm-frame` instead of
    document-wide (the step keys "system"/"code" are now on a page that has both).
  - **Verified in-browser on all 3 pages**, light and dark: node counts match the old
    standalone pages exactly (agentic 16/19, predictops 15, inference-lens 20), 6 flow chips
    per diagram, chip-spotlight adds/removes `.hl`, zero text overflowing any box, packets
    animate only in the on-screen diagram, and node fills flip with the theme. New assets are
    at `?v=1`; `style.css`/`script.js` were not edited so their `?v=4` is unchanged.
  - **Not committed** (ask-before-committing rule). NOTE: this landed on `main`, which still
    carries the retired `+30.3pp` and `~60-65% crossover` claims — including inside the
    Multi-Agent system map's "Inverse Capability Hypothesis" node, which is now on the project
    page rather than a separate blueprint page. Unrelated to this change, but it ships wider now.
